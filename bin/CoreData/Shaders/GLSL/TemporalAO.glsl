...
vec2 getCameraMoveOffset(vec2 offset)
{
	vec3 posInView = fetch_eye_pos(offset);
...
void PS()
{
	...
	vec2 ss_vel = getCameraMoveOffset(c_frag.xy);
	vec4 depthInput = texture2D(sDepthBuffer, uv);
	#ifdef HWDEPTH
    ...
    #endif
#else
	vec2 ss_vel = getCameraMoveOffset(uv);
	vec4 depthInput = texture2D(sDepthBuffer, uv);
	#ifdef HWDEPTH
